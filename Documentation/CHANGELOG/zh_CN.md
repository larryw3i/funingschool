<hr/>
<p align="center">    <a href="https://gitee.com/larryw3i/funingschool/blob/master/Documentation/CHANGELOG/zh_CN.md">简体中文</a> •    <a href="https://github.com/larryw3i/funingschool/blob/master/Documentation/CHANGELOG/en_US.md">English</a></p>

# 更新日志

## [版本 20260617.82119.813] - 2026年06月17日
### 新增

### 更改

### 弃用

### 修复
- `食堂/编辑食材`页面：修复了编辑食材的时候不显示没有食材的大类（无食材大类）的问题。
- `食堂/编辑食材`页面：修复了编辑大类名为空的食材的时候不自动更新大类的问题，如果有其它相同名称的食材的大类的名称为空字符，也按食材名称自动更新。
### 移除
- `食堂/编辑大类`页面：移除食材大类的“pin_to_consumptions_top”属性。
## [版本 20260607.80837.824] - 2026年06月07日
### 新增

### 更改

### 弃用

### 修复
- `食堂/出库` 页面：修复了排序后无法更新消耗数量的问题。
- `食堂/出库` 页面：让最小消耗数量为 `0` ，而且不能为负数。
- `食堂/出库` 页面：修复了删除已有消耗数量后更新消耗数量失败的情况。
- `食堂/出库` 页面：修复了出库表的表头和列不匹配的情况。
### 移除

## [版本 20260601.81344.845] - 2026年06月01日
### 新增
- 搜索食材：可以在输入框搜索数量和总价。
- 服务协议：增加“服务协议”页面。
- 排序：增加“大类”和“餐类”排序功能。
- 注册：添加“自动生成密码”功能。
### 更改

### 弃用

### 修复

### 移除

## [版本 20260525.82149.841] - 2026年05月25日
### 新增
- 可选“站点”功能：可启用“站点”功能，该功能包括注册邮箱验证、登录通知、密码重置等。
- 筛选食材：在“list_ingredients”和“create_consumptions”页面提供了更加便利的食材筛选条件。
- 编辑食材：空“大类”的食材、相同“食材名”的食材会自动更新。
- 主题：按照系统主题自动启用页面主题。
### 更改

### 弃用

### 修复

### 移除

## [版本 20260422.82120.845] - 2026年04月22日
### 新增
- 工作簿文件：为生成的工作簿文件添加一些`属性`。
### 更改
- 生成工作簿文件：给生成文件的文件名添加`时间戳`。
- `食材消耗`页面：点击排序条件按钮时直接在表格里进行排序，不再刷新页面。
### 弃用

### 修复

### 移除

## [版本 20260415.80820.815] - 2026年04月15日
### 新增
- `食材消耗`页面：输入时，让表格中的输入框在被表头或固定在左边的列遮住时自动滚动到可视区域。
### 更改
- 启动：在`Linux 发行版`上启动的时候会出现浏览器已打开但无法访问`已指定的 URL`的情况，因此启动的时候适当延迟打开浏览器。
### 弃用

### 修复
- 编辑食材：修复了有 大类的 大类名 为`空`时`编辑食材`页面无法被加载的问题。
### 移除

## [版本 20260409.80155.835] - 2026年04月09日
### 新增

### 更改
- `食材列表`页面：让总价的汇总信息更直观。
### 弃用

### 修复
- 新增食材：更新“new_ingredients”函数，防止在餐类为空的情况下向数据库中写入多个餐类实例。
- 食材消耗：修复了 `食材消耗` 页面的食材进度条的显示问题。
### 移除

## [版本 20260127.80117.831] - 2026年01月27日
### 新增
- 添加 **CHANGELOG.md**： 为 `CHANGELOG.md` 添加多语言特性。
- 删除食材： 添加`批量删除`食材的功能。
### 更改

### 弃用
- “202511012053_copy_profiles_to_fnprofile”补丁：2026 年 02 月后，它将被弃用。
### 修复

### 移除


