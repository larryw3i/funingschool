...
class Build:
    ...
    def get_dists_hash(self):
        sh_value = (
            f"cd {dist_dpath}; " + f"sha256sum *; " + f"cd {project_dpath};"
        )
        hash_value = subprocess.check_output(
            sh_value, shell=True, universal_newlines=True
        )
        return hash_value
    ...
    def build(self, upload=False):
        ...
        sh_value = f"rm -rf {dist_fpaths}; " + f"python -m build;"
        sh(sh_value)

        if upload:
            sh_value = f"python -m twine upload {dist_fpaths}"
            sh(sh_value)
            ...
            print(
                ...
                if len(os.listdir(dist_dpath)) == 1
                else _("The hash values of distributions are:")
            )
            print(hash_value)
